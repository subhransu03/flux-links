...
export function ShortcutCard({ shortcut, onEdit, onDelete, isDragging = false }: ShortcutCardProps) {
  ...
  return (
    <Card className={`group aspect-square relative overflow-hidden bg-card backdrop-blur-sm border-border/60 hover:border-primary/50 transition-all duration-300 ease-in-out transform hover:-translate-y-1 shadow-lg hover:shadow-primary/20 hover:animate-glow cursor-pointer 
      ...
      `}
    >
      <a href={shortcut.url} target="_blank" rel="noopener noreferrer" className="block p-4 h-full" draggable="false" onClick={(e) => { if(isDragging) e.preventDefault() }}>
